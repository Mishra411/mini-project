...
def get_session_no(conn, cid):
    ...
    if last_row and last_row["last_session"]:
        last = last_row["last_session"] 
    else:
        last = 0
...
def search_products(conn, cid, session_no):
    ...
    while True:
        ...
        for row in rows:
            ...
            descr = row["descr"]
            print(f"{pid}: {name} - ${price} ({stock} in stock)")
...
def add_to_cart(conn, cid, session_no, pid):
    ...
    stock = product_check["stock_count"]
    ...
    while True:
        ...
        for order in orders:
            ...
            total_row = execute_query(
                conn,
                "SELECT SUM(qty * uprice) AS total FROM orderlines WHERE ono=?",
                (ono,),
                fetchone=True
            )
            total = 0
            if total_row and total_row["total"]:
                ...
                total = 0
            print(f"{ono}: Date {odate} | Ship to: {address} | Total: ${total:.2f}")

        nav = input("\nEnter Order No to view, N for next, P for prev, Q to quit: ").strip().lower()
        if nav == "n":
            page += 1
        elif nav == "p" and page > 0:
            page -= 1
...
def view_order_detail(conn, cid, ono):
    ...
    grand_total = 0
    for row in lines:
        print(f"{row['name']} ({row['category']}) | {row['qty']} x ${row['uprice']:.2f} = ${row['total']:.2f}")
